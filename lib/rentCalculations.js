...
export const getCurrentMonthRentMetrics = (employees, invoices, year, month) => {
  ...
  // 4. Filter for issued invoices only (已開出)
  const issuedInvoices = currentMonthInvoices.filter(inv => inv.is_issued === true);
  
  // 5. Invoiced Rent = Total amount from issued invoices
  const invoicedRent = issuedInvoices.reduce((sum, inv) => sum + (parseFloat(inv.amount) || 0), 0);
  
  // 6. Not Yet Received = Pending/Due/Overdue invoices that are ISSUED
  const notYetReceivedRent = issuedInvoices
