...
export const getCurrentMonthRentMetrics = (employees, invoices, year, month) => {
  // 1. Total Receivable Rent = Projected income (housed + prorated upcoming)
  const totalReceivableRent = calculateProjectedIncome(employees, year, month);
  ...
  // 3. Filter for issued invoices only and calculate proportional amounts
  const issuedInvoices = currentMonthInvoices.filter(inv => inv.is_issued === true);
  
  // 4. Calculate proportional amounts for each issued invoice
  let invoicedRent = 0;
  let receivedRent = 0;
  
  // Debug logging for September (month 8, 0-based)
  const isDebugMonth = (year === 2025 && month === 8);
  if (isDebugMonth) {
    console.log(`🔍 DEBUG: September 2025 Dashboard Calculation`);
    console.log(`📊 Total current month invoices: ${currentMonthInvoices.length}`);
  ...
  issuedInvoices.forEach((inv, index) => {
    ...
    // Find employee for monthly rent
    const employee = employees.find(emp => emp.id === inv.employee_id);
    const employeeMonthlyRent = parseFloat(employee?.rent || employee?.monthlyRent || 0);
    
    // Calculate proportional amount for this month
    const proportionalAmount = calculateProportionalAmount(
      startDate, endDate, year, month, totalAmount, employeeMonthlyRent
    );
    
    // Debug logging for September
    if (isDebugMonth && index < 10) {
      console.log(`📊 Invoice ${index + 1}: ${inv.invoice_number}`, {
        amount: totalAmount,
    ...
    // Add to totals
    invoicedRent += proportionalAmount;
    
    if (inv.status === 'paid') {
      receivedRent += proportionalAmount;
      if (isDebugMonth) {
        console.log(`✅ PAID: Added ${proportionalAmount.toFixed(2)} to receivedRent, total now: ${receivedRent.toFixed(2)}`);
      }
    }
  });
  
  if (isDebugMonth) {
    console.log(`📊 SEPTEMBER TOTALS:`, {
      invoicedRent: invoicedRent.toFixed(2),
      receivedRent: receivedRent.toFixed(2),
      totalIssuedInvoices: issuedInvoices.length,
      paidInvoicesCount: issuedInvoices.filter(inv => inv.status === 'paid').length
    });
  }
  
  // 5. Not Yet Received = Invoiced Rent - Received Rent (待收款發票金額)
  const notYetReceivedRent = invoicedRent - receivedRent;
  
  // 6. Collection rate based on proportional amounts
  const collectionRate = invoicedRent > 0 ? (receivedRent / invoicedRent * 100) : 0;
