...
export const getCurrentMonthRentMetrics = (employees, invoices, year, month) => {
  ...
  // 3. Filter for issued invoices only (已開出)
  const issuedInvoices = currentMonthInvoices.filter(inv => inv.is_issued === true);
  
  // 4. Invoiced Rent = Total amount from issued invoices (已產生發票的租金總額)
  const invoicedRent = issuedInvoices.reduce((sum, inv) => sum + (parseFloat(inv.amount) || 0), 0);
  
  // 5. Received Rent = Paid issued invoices only (已收租金)
  const receivedRent = issuedInvoices
    .filter(inv => inv.status === 'paid')
    .reduce((sum, inv) => sum + (parseFloat(inv.amount) || 0), 0);
  
  // 6. Not Yet Received = Invoiced Rent - Received Rent (待收款發票金額)
  const notYetReceivedRent = invoicedRent - receivedRent;
  
  // 7. Collection rate based on issued invoices
