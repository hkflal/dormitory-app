...
const getStatusBadge = (status, isIssued, onIssuedChange, invoiceId) => {
        ...
        return (
            <div className="flex items-center space-x-2">
                ...
                <label className="flex items-center space-x-1 cursor-pointer">
                    <input
                        type="checkbox"
                        checked={isIssued || false}
                        onChange={(e) => onIssuedChange && onIssuedChange(invoiceId, e.target.checked)}
                        className="h-3 w-3 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
                    <span className="text-xs text-gray-600 dark:text-gray-400">已開出</span>
                </label>
            </div>
        );
    }
...
const InvoiceCard = ({
    ...
    const invoiceCompany = invoice.company || '';
        ...
        return (
            <div className="px-3 py-2 my-1 bg-white rounded shadow-sm dark:bg-gray-800 border-l-4 border-primary-500 flex items-center min-h-[56px]">
                <div className="flex-1 min-w-0 pr-2">
                    ...
                    <span className="mt-1 block">{getStatusBadge(invoice.status, invoice.is_issued, handleIssuedChange, invoice.id)}</span>
                </div>
                <button
    ...
    return (
        <div className="px-3 py-2 my-1 bg-white rounded shadow-sm dark:bg-gray-800 flex items-center min-h-[56px]">
            <div className="flex-1 grid grid-cols-7 gap-2 items-center">
                ...
                <span>{getStatusBadge(invoice.status, invoice.is_issued, handleIssuedChange, invoice.id)}</span>
            </div>
...
const InvoicesPage = () => {
    ...
    const handleAddOrUpdateInvoice = async () => {
        ...
        // No need to save again, just refresh the list
        fetchInvoices();
    };
    ...
    const handleNavigateToDetail = async () => {
        ...
        try {
            // Check if CTR exists in invoices collection
            ...
            // Check if CTR exists in employees collection
            const employeesSnapshot = await getDocs(collection(db, 'employees'));
            const employeesWithCTR = employeesSnapshot.docs.filter(doc => {
                const data = doc.data();
                return data.contractNumber === ctrNumber || 
                       data.activeCtr === ctrNumber ||
                       data.contract_number === ctrNumber;
            });

            // If CTR exists in either collection, navigate to detail page
            if (!invoicesSnapshot.empty || employeesWithCTR.length > 0) {
                router.push(`/invoice-detail/${ctrNumber}`);
            } else {
                alert('無此合約編號');
            }
        } catch (error) {
            console.error('Error checking CTR:', error);
            alert('查詢合約編號時發生錯誤');
        }
    };
    ...
    const handleIssuedChange = async (invoiceId, isIssued) => {
        if (updatingIssued[invoiceId]) return;
        setUpdatingIssued(prev => ({ ...prev, [invoiceId]: true }));
        try {
            const invoiceRef = doc(db, 'invoices', invoiceId);
            await updateDoc(invoiceRef, { is_issued: isIssued });
            setInvoices(prevInvoices =>
                prevInvoices.map(inv => inv.id === invoiceId ? { ...inv, is_issued: isIssued } : inv)
            );
        } catch (error) {
            console.error('Failed to update issued status', error);
        } finally {
            setUpdatingIssued(prev => ({ ...prev, [invoiceId]: false }));
        }
    };
    ...
    const filteredInvoices = invoices.filter(invoice => {
        ...
        return (
            ...
            ((invoice.company || '').toLowerCase().includes(searchTermLower))
        );
    });
        ...
        <div className="p-4 sm:p-6 lg:p-8">
            <div className="sm:flex sm:items-center sm:justify-between mb-6">
                ...
                <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none flex items-center space-x-2">
                    <button onClick={() => setShowExportModal(true)} className="bg-blue-600 text-white px-4 py-2 rounded-md shadow-sm hover:bg-blue-700 flex items-center focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 h-9">
                    ...
                    <div className="border border-t-0 border-gray-200 dark:border-gray-700 rounded-b-lg p-2 bg-gray-50 dark:bg-gray-900">
                        {grouped[group].map(invoice => (
                            <InvoiceCard 
                                ...
                                handleIssuedChange={handleIssuedChange}
