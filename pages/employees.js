...
export default function Employees() {
  ...
  const openEditModal = (employee) => {
    ...
    setEmployeeForm({
      name: employee.name || '',
      contract_number: employee.contract_number || employee.contractNumber || employee.activeCtr || '',
          ...
          return keys.map(key => (
            <div key={key} className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
              <div className="px-4 sm:px-6 py-4 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">{key} <span className={`px-3 py-1 text-sm rounded-full ${groupBy === 'property' && getGroupOccupancy(key, grouped[key]) >= getPropertyCapacity(key)
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">UID / 合約</th>
                      ...
                      <tr key={employee.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col space-y-1">
                            <div className={`text-xs font-mono text-center text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded border-2 inline-block ${employee.gender === 'female' ? 'border-pink-300 dark:border-pink-500 shadow-lg shadow-pink-200 dark:shadow-pink-800' :
                            ...
                            {employee.contract_number && (
                              <div className="text-xs font-mono text-center bg-blue-50 text-blue-700 border border-blue-200 px-2 py-1 rounded dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800 inline-block">
                                {employee.contract_number}
                              </div>
                            )}
                          </div>
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input type="text" id="contract_number" value={employeeForm.contract_number || ''} onChange={handleInputChange} className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600" placeholder="例: D10250" />
        </div>
        <div>
